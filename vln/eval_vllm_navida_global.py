...
def evaluate_agent(result_queue, base_url, model_name, config, dataset, result_path, num_generations,
    ...
    for target_ep in episodes_snapshot:
        for trial_id in range(pass_k):
            ...
            t_dict["t_episode"] = time.time() - episode_start_time
            t_dict["scene_id"] = scene_id
            t_dict["episode_id"] = str(episode_id)
            t_dict["episode_instruction"] = episode_instruction
            t_dict["success"] = float(info["success"])
            t_dict["spl"] = float(info["spl"])
            t_dict["os"] = float(info["oracle_success"])
            t_dict["ne"] = float(info["distance_to_goal"])
            result_queue.put(t_dict)
...
def main():
    ...
    # Live aggregation:
    #   avgSR  = mean success over every trial run
    #   pass@K = mean over trajectories of max(success across that traj's trials)
    pass_at_k_key = f"pass@{int(args.pass_k)}"
    n_eval = 0
    sum_suc = 0.0
    sum_spl = 0.0
    sum_ne = 0.0
    ...
    def _ingest_live(r):
        nonlocal n_eval, sum_suc, sum_spl, sum_ne
        if "success" not in r or "scene_id" not in r or "episode_id" not in r:
            return
        n_eval += 1
        suc = float(r["success"]); spl = float(r["spl"]); ne = float(r["ne"])
        sum_suc += suc; sum_spl += spl; sum_ne += ne
        key = (r["scene_id"], str(r["episode_id"]), r.get("episode_instruction", ""))
        if key not in traj_live:
            traj_live[key] = suc
        else:
            traj_live[key] = max(traj_live[key], suc)

    # Seed from any pre-existing results so resume keeps live numbers correct.
    _seed = os.path.join(args.result_path, "result.json")
        ...
        with open(_seed) as _f:
            for _line in _f:
                ...
                try:
                    ...
                    continue
                if "episode_instruction" in _r and "success" in _r:
                    _ingest_live(_r)
        ...
        while done_count < target:
            ...
            try:
                ...
                if not result.get("skipped"):
                    _ingest_live(result)
                if n_eval:
                    passk = sum(traj_live.values()) / len(traj_live)
                    pbar.set_postfix({
                        "avgSR": f"{sum_suc / n_eval * 100:.2f}%",
                        pass_at_k_key: f"{passk * 100:.2f}%",
                        "mSPL": f"{sum_spl / n_eval:.3f}",
                        "mNE": f"{sum_ne / n_eval:.2f}",
                        "n": n_eval,
                    })
            except Exception:
                if not alive:
