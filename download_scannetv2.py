...
import tempfile
try:
    from urllib.request import urlretrieve, urlopen  # python3
except ImportError:
    from urllib import urlretrieve, urlopen  # python2.7
...
def get_release_scans(release_file):
    ...
    for scan_line in scan_lines:
        s = scan_line.decode('utf8') if hasattr(scan_line, 'decode') else scan_line
        scan_id = s.rstrip('\n')
...
def main():
    ...
    if args.task_data:  # download task data
        ...
        frame_url = BASE_URL + RELEASE_TASKS + '/' + PREPROCESSED_FRAMES_FILE[0]
        download_file(frame_url, os.path.join(out_dir_tasks, PREPROCESSED_FRAMES_FILE[0]))
