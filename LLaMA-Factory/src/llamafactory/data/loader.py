...
from .lazy import LazyMapDataset
...
def _get_preprocessed_dataset(
    ...
    if data_args.lazy_map_dataset:
        if data_args.streaming:
            raise ValueError("`lazy_map_dataset` is incompatible with `streaming`.")
        if data_args.packing:
            raise ValueError("`lazy_map_dataset` is incompatible with `packing`.")
        if stage == "kto":
            raise ValueError("`lazy_map_dataset` is currently unsupported for `kto` stage.")
        ...
        lazy_dataset = LazyMapDataset(dataset=dataset, dataset_processor=dataset_processor)
        if training_args.should_log:
            try:
                print("eval example:" if is_eval else "training example:")
                dataset_processor.print_data_example(lazy_dataset[0])
...
def get_dataset(
    ...
    if data_args.tokenized_path is not None:
        ...
        if data_args.lazy_map_dataset:
            raise ValueError("`tokenized_path` is incompatible with `lazy_map_dataset`.")
    ...
    with training_args.main_process_first(desc="pre-process dataset", local=(not data_args.data_shared_file_system)):
        ...
        if data_args.lazy_map_dataset:
            dataset_module = {}
            if "train" in train_dict:
                dataset_module["train_dataset"] = train_dict["train"]

            if "validation" in eval_dict:
                dataset_module["eval_dataset"] = eval_dict["validation"]
            else:
                eval_dataset = {}
                for key in eval_dict:
                    if key.startswith("validation_"):
                        eval_dataset[key[len("validation_") :]] = eval_dict[key]
                if len(eval_dataset):
